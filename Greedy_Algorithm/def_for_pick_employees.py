...
def is_eligible_for_event(emp_id: int, event_id: int, dict_events, employee_days_off, employee_worked_days, daily_hours_per_employee, 
    ...
    if not respects_max_days(emp_id, blocked_days, employee_worked_days):
        return False
    ...
    if not respects_min_rest(emp_id, shift_begins, shift_ends, min_rest_hours, assigned_shifts):
        return False
    ...
    for event_id, state in event_state.items():
        for role in state["roles"]:
            if role["filled_by"] is not None:
                continue
            if not is_eligible_for_event(emp_id, event_id, dict_events, employee_days_off, employee_worked_days, daily_hours_per_employee, 
                          max_daily_hours, hours_per_employee, period_weeks, max_weekly_hours, min_rest_hours, assigned_shifts):
                continue
            current_team = [r["filled_by"] for r in event_state[event_id]["roles"] if r["filled_by"] is not None]
            if not is_valid_final_team(current_team + [emp_id], dict_employees):
                continue

            score = personal_role_score(emp_id, event_id, role, dict_events, dict_employees, score_rules, skillset_scores, event_requests)
